...
def Exit():
    result = tkMessageBox.askquestion('Marks Holder', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
def ShowLoginForm():
    global loginform
    loginform = Toplevel()
    loginform.title("Marks Holder/Account Login")
    width = 600
    height = 500
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
...
def Home():
    ...
    Title.pack(pady=10)
    lbl_display = Label(Title, text="Marks Holder", font=('arial', 45))
    lbl_display.pack()
    menubar = Menu(Home)
...
def ShowAddNew():
    global addnewform
    addnewform = Toplevel()
    addnewform.title("Marks Holder/Add new")
    width = 600
    height = 500
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def AddNewForm():
    ...
    lbl_text = Label(TopAddNew, text="Add New Student", font=('arial', 18), width=600)
    lbl_text.pack(fill=X)
    MidAddNew = Frame(addnewform, width=600)
    MidAddNew.pack(side=TOP, pady=50)
    lbl_productname = Label(MidAddNew, text="Student Name:", font=('arial', 25), bd=10)
    lbl_productname.grid(row=0, sticky=W)
    lbl_qty = Label(MidAddNew, text="Serial No.:", font=('arial', 25), bd=10)
    lbl_qty.grid(row=1, sticky=W)
    lbl_price = Label(MidAddNew, text="Student Marks:", font=('arial', 25), bd=10)
...
def ViewForm():
    ...
    lbl_text = Label(TopViewForm, text="View Students", font=('arial', 18), width=600)
    lbl_text.pack(fill=X)
    ...
    tree = ttk.Treeview(MidViewForm, columns=("StudentID", "Student Name", "Serial no.", "Student Marks"), selectmode="extended", height=100, yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
    ...
    tree.heading('Serial no.', text="Serial No.",anchor=W)
    tree.heading('Student Marks', text="Student Marks",anchor=W)
...
def Delete():
    if not tree.selection():
       print("ERROR")
        ...
        result = tkMessageBox.askquestion('Marks Holder', 'Are you sure you want to delete this record?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
...
def ShowView():
    global viewform
    viewform = Toplevel()
    viewform.title("Marks Holder/View Marks")
    width = 600
    height = 400
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def Logout():
    result = tkMessageBox.askquestion('Marks Holder', 'Are you sure you want to logout?', icon="warning")
    if result == 'yes': 
        admin_id = ""
        root.deiconify()
        Home.destroy()
